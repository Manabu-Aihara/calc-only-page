...
from app.models import Attendance, StaffJobConstract
...
class PandaCatPandas:
    ...
    def get_filter(self) -> list:
        ...
        attendance_filters.append(
            Attendance.WORKDAY.between(self.from_day, self.to_day)
        )
    ...
    async def calc_attendance_of_month(self):
        ...
        stmt = (
            select(Attendance, StaffJobConstract.CONTRACT_CODE)
            .filter(*filter)
            .join(
                StaffJobConstract,
                StaffJobConstract.STAFFID == StaffJobConstract.STAFFID,
            )
        )
        ...
        for month_attend in month_attend_list:
            ...
            sum_over: int = sum(over_times)
            o_h = sum_over // (60 * 60)
            o_m = (sum_over - o_h * 60 * 60) // 60
            # 【項目17】残業時間計：60進数
            over_60 = o_h + o_m / 100

            over_10 = sum_over / (60 * 60)
            # 【項目18】残業時間計：10進数
