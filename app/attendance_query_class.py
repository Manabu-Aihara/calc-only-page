...
class AttendanceQuery:
    ...
    def _get_template_filter(self):
        ...
        # attendance_filters.append(StaffJobContract.START_DAY <= self.filter_from_day)  # 2
        attendance_filters.append(StaffJobContract.END_DAY >= self.filter_to_day)  # 3
        attendance_filters.append(
            StaffJobContract.JOBTYPE_CODE == CollrateralTemplate.JOBTYPE_CODE
        )
        attendance_filters.append(
            StaffJobContract.CONTRACT_CODE == CollrateralTemplate.CONTRACT_CODE
        )
        return attendance_filters
    ...
    def get_templates(self):
        ...
        template_filters = self._get_template_filter()
        return (
            db.session.query(CollrateralTemplate.TEMPLATE_NO)
            # .join(
            #     StaffJobContract,
            #     # どっちでもいいと思う
            #     StaffJobContract.JOBTYPE_CODE == CollrateralTemplate.JOBTYPE_CODE,
            #     # StaffJobContract.CONTRACT_CODE == CollrateralTemplate.CONTRACT_CODE,
            # )
            .filter(and_(*template_filters)).group_by(CollrateralTemplate.TEMPLATE_NO)
        )
    ...
    def get_attendance_query(self):
        ...
        return (
            db.session.query(
                ...
                CollrateralTemplate.TEMPLATE_NO,
